...
class User(Base):
    __tablename__ = "users"
    __table_args__ = (
        CheckConstraint(_in_constraint("role", _ROLE_VALUES), name="ck_users_role"),
        CheckConstraint("is_active IN (0, 1)", name="ck_users_is_active"),
        Index("idx_users_username", "username", unique=True),
        Index("idx_users_role_barangay", "role", "assigned_barangay"),
    )
    ...
    assigned_barangay: Mapped[str | None] = mapped_column()
    is_active: Mapped[int] = mapped_column(default=1)
