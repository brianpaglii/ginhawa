...
from ginhawa_cloud.db.models import AuditLog, Citizen, Measurement, User
...
# + 3 BHWs = 4), citizens (20), sessions (5), measurements (15), and at
# least 44 audit rows (one create row per seeded entity).
...
def test_seed_creates_expected_counts(db_session: Session) -> None:
    ...
    # 4 user creations + 20 citizens + 5 sessions + 15 measurements = 44.
    # Use >= to allow for additional audit rows the helper may emit
    # (none today, but keeps the test robust to future record_audit
    # behaviour changes).
    assert _scalar_count(db_session, AuditLog) >= 44
